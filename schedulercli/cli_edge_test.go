...
func TestCLIUsageAndRuntimeFailures(t *testing.T) {
	...
	tests := []struct {
		...
		{[]string{"due", "--after", "2026-01-01T00:00:00Z"}, 2},
		{[]string{"due", "--name", "report", "--unknown"}, 2},
		...
		{[]string{"test", "--at", "2026-01-01T00:00:00Z"}, 2},
		{[]string{"test", "--name", "report", "--unknown"}, 2},
		{[]string{"test", "--name", "report", "--at", "bad"}, 2},
		{[]string{"test", "--name", "missing", "--at", "2026-01-01T00:00:00Z"}, 1},
		{[]string{"recover", "--unknown"}, 2},
		{[]string{"recover", "--token", "1"}, 2},
		{[]string{"recover", "--key", "key", "--unknown"}, 2},
